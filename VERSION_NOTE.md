# SecPlus Sensei — version & changelog

**Bump the version after every code change.**
...
| File | What to update |
|------|----------------|
| `CURRENT_VERSION.txt` | Single line: `1.1.7` |
| `package.json` | `"version": "1.1.7"` |
| `secplus-sensei.html` | `var APP_VERSION="1.1.7"` |
...
- v1.1.1: Compact header nav (tabs no longer stretch full width). Persistent "Unofficial study aid" disclaimer in a new app footer on every view; new About & Help modal (version, what the app does, how optional tutoring works, privacy note, FAQ/known issues); all 5 raw API error messages (Optimize, Card tutor x2, quiz explain, refine suggestions) replaced with friendly `friendlyAiError()` classification (bad key / offline / rate-limited / out of credits / other); fixed stale "check API key on the Dashboard" text left over from the old redirect-based flow; fixed "Refine suggestions" missing loading/disabled state (could double-fire) and dropped its redundant pre-emptive toast; repo hygiene — deleted stale `dist2`–`dist6` build-output folders (~37MB, mostly duplicate Chromium license files), `.gitignore` now ignores `dist*/`; reformatted `README.txt` spacing.
- v1.0.9: Priority 1 (onboarding/BYOK) — first-run welcome modal explaining offline-first + optional tutoring, with CompTIA disclaimer; replaced disruptive "redirect to Dashboard" API-key prompt with an in-place quick-add key modal on all 4 tutor entry points (Optimize, Card tutor, quiz explain, refine suggestions); Dashboard tutor card reworded as clearly optional with a "How does this work?" link back to the welcome modal.
